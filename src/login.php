    ...
    while($row = $result->fetch_assoc())
    {
        if ($row['username'] == $login['user'])
        {
            if ($row['hash'] == strtoupper(hash('sha512', $login['pass'] . $row['salt'])))
            {
                ...
                $sql = "INSERT INTO `sessions` (`sessionId`, `userId`, `lastPing`, `validUntil`) VALUES ('" . $_SESSION['sessionId'] . "', '" . $row['userId'] . "', '" . date("H:i:s Y-m-d", strtotime('+0hour')) . "', '" . date("H:i:s Y-m-d", strtotime('+1 hour')) . "');";
