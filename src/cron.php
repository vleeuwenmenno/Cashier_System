<?php
    ...
    while($rows = $results->fetch_assoc())
    {
        ...
        if ($diffDays != 0)
        {
            echo $rows['contractId']." was ingepland voor factuur datum ".$rows['orderDate'].' maar versturen was mislukt.<br/>';
            Contract::sendOrder($rows['contractId'], $rows['orderDate']);
        }
    }
    ...
    while($rows = $results->fetch_assoc())
    {
        ...
        $date = $today->format("Y").'-'.(str_pad(($today->format("m")-1), 2, '0', STR_PAD_LEFT)).'-'.$pd;
        ...
        if (new DateTime($rows['startDate']) < new DateTime($date))
        {
            ...
            if (!$found)
            {
                echo "$cid was ingepland voor vorige maand op factuur datum $date maar was nooit verstuurd, wordt nu verstuurd ... <br />";
                Contract::sendOrder($rows['contractId'], $date);
            }
        }
    }
    ...
    {        
        ...
        if ($diffDays == 0)
        {
            ...
            echo "$cid is ingepland voor vandaag ";
            ...
            {     
                $found = true;
                echo ' maar is al geregistreerd als verzonden ';

                /// If it failed to send try sending it again!
                if ($row['success'] == false)
                {
                    echo 'maar dit was mislukt, wordt nu opnieuw verstuurd ...<br/>';
                    Contract::sendOrder($row['contractId'], $orderDate);
                }
                else
                    echo '.<br/>';
            }
            ...
            if (!$found)
            {
                echo "wordt nu verstuurd ...<br/>";
                Contract::sendOrder($rows['contractId'], $orderDate);
            }
        }
    }