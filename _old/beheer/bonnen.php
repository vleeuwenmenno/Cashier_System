...
function process_input_bonnen()
{
  ...
  elseif( $submit == "Alles" )
  {
     ...
     $query = "select klant.id as klantid, bon.id as id, bon.status as status, bon.datum as datum, bon.tijd as tijd, bon.totaal as totaal, klant.postcode as postcode, klant.achternaam as achternaam, bon.betaalwijze as betaalwijze from bon LEFT JOIN klant ON bon.klantid = klant.id order by bon.datum desc";
     $subpage = SMAIN;
  }
  elseif( $submit == "Reset" )
  {
...
{ 
  ...
  display_beheer_menu(MBONNEN, SMAIN);
  display_bonnen_main();
  echo '</div>';
  echo '</body>';
  echo '</html>';
...
function display_bonnen_query()
{
  ...
  echo '<form action="', "${httpdir}", '/beheer/bonnen.php" method="post">', NL;
...
function display_result_row($row, $i)
{
    ...
    if( $row->status == 'wacht' )
    {
    	echo "<TD><input class=resultbutton type=button value=\"Verwijderen\" onClick=\"verwijderen_bon_beheer($i)\"</TD>";
    }
    else
    {
    	echo "<TD> </TD>";
    ...
    echo "<TD><input class=resultbutton type=button value=\"Openen\" onClick=\"nieuwe_bon($i)\"</TD>";
    echo "</TR>";
...
{
  global $idbon, $postcode, $achternaam;
